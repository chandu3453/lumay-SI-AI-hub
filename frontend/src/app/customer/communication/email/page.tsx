...
export default function CustomerEmailPage() {
  ...
  const startSession = async () => {
    try {
      const res = await interactionsService.start("cust-102", "email");
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-4 border-b border-[#E5E7EB] bg-blue-50/50 cursor-pointer relative">
            ...
            <p className="text-[11px] text-slate-500 line-clamp-2 leading-relaxed">
              Dear Fatima, Welcome to LuMay Insurance email support. We have received your inquiry...
            </p>
          </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-baseline gap-2">
                        <p className="text-sm font-bold text-[#0D1B3E]">{m.role === "user" ? "Fatima Al Lawati" : "LuMay Support"}</p>
                        <p className="text-xs text-slate-500">&lt;{m.role === "user" ? "fatima.lawati@email.com" : "support@lumay.com"}&gt;</p>
                      </div>
                      <p className="text-[11px] text-slate-400 mt-0.5">To: {m.role === "user" ? "support@lumay.com" : "fatima.lawati@email.com"}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
